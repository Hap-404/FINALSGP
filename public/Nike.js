...
window.onload = function () {
  ...
  revenueChart = createChart('myChart', [37.4, 39.1, 44.5, 46.3, 51.2]);
  profitChart = createChart('myChart1', [4.0, 4.2, 2.5, 5.7, 6.0]);
...
function toggleGraph() {
  ...
  async function handlePayment(amount) {
    try {
        const response = await fetch("http://localhost:3000/create-checkout-session-nike", {
            method: "POST",
  ...
  document.getElementById("buyBtn").addEventListener("click", async function () {
    ...
    const response = await fetch("/create-checkout-session-nike", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
