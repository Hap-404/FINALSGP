...
window.onload = function () {
  ...
  revenueChart = createChart('myChart', [11.3, 13.0, 17.4, 31.8, 37.3]);
  profitChart = createChart('myChart1', [11.3, 13.0, 17.4, 31.8, 37.3]);
...
function toggleGraph() {
  ...
  async function handlePayment(amount) {
    try {
        const response = await fetch("http://localhost:3000/create-checkout-session-uber", {
            method: "POST",
  ...
  document.getElementById("buyBtn").addEventListener("click", async function () {
    ...
    const response = await fetch("/create-checkout-session-uber", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
