...
window.onload = function () {
  ...
  revenueChart = createChart('myChart', [524.0, 559.2, 572.8, 611.3, 648.1]);
  profitChart = createChart('myChart1', [14.9, 13.7, 13.6, 11.7, 15.5]);
...
function toggleGraph() {
  ...
  async function handlePayment(amount) {
    try {
        const response = await fetch("http://localhost:3000/create-checkout-session-walmart", {
            method: "POST",
  ...
  document.getElementById("buyBtn").addEventListener("click", async function () {
    ...
    const response = await fetch("/create-checkout-session-walmart", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
