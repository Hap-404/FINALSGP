...
window.onload = function () {
  ...
  revenueChart = createChart('myChart', [31.5, 53.8, 81.5, 96.8, 97.7]);
  profitChart = createChart('myChart1', [0.721, 5.5, 12.6, 15.0, 7.1]);
...
function toggleGraph() {
  ...
  async function handlePayment(amount) {
    try {
        const response = await fetch("http://localhost:3000/create-checkout-session-tesla", {
            method: "POST",
  ...
  document.getElementById("buyBtn").addEventListener("click", async function () {
    ...
    const response = await fetch("/create-checkout-session-tesla", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
