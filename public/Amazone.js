...
window.onload = function () {
  ...
  revenueChart = createChart('myChart', [386.1, 469.8, 513.9, 574.7, 637.9]);
  profitChart = createChart('myChart1', [21.3, 33.3, -2.7, 30.4, 59.2]);
...
window.addEventListener("storage", function(event) {
  ...
  async function handlePayment(amount) {
    try {
        const response = await fetch("http://localhost:3000/create-checkout-session-amazone", {
            method: "POST",
  ...
  document.getElementById("buyBtn").addEventListener("click", async function () {
    ...
    const response = await fetch("/create-checkout-session-amazone", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
