...
window.onload = function () {
  ...
  revenueChart = createChart('myChart', [39.5, 40.5, 42.4, 44.2, 49.9]);
  profitChart = createChart('myChart1', [11.0, 10.1, 10.2, 8.5, 9.6]);
...
function toggleGraph() {
  ...
  async function handlePayment(amount) {
    try {
        const response = await fetch("http://localhost:3000/create-checkout-session-oracle", {
            method: "POST",
  ...
  document.getElementById("buyBtn").addEventListener("click", async function () {
    ...
    const response = await fetch("/create-checkout-session-oracle", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
