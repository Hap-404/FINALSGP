...
window.onload = function () {
  ...
  revenueChart = createChart('myChart', [143.02, 168.09, 198.27, 211.92, 245.12]);
  profitChart = createChart('myChart1', [44.28, 61.27, 72.74, 72.36, 88.14]);
...
window.addEventListener("storage", function(event) {
  ...
  async function handlePayment(amount) {
    try {
        const response = await fetch("http://localhost:3000/create-checkout-session-microsoft", {
            method: "POST",
  ...
  document.getElementById("buyBtn").addEventListener("click", async function () {
    ...
    const response = await fetch("/create-checkout-session-microsoft", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
