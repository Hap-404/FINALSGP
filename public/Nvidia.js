...
window.onload = function () {
  ...
  revenueChart = createChart('myChart', [16.7, 26.9, 27.0, 44.9, 60.9]);
  profitChart = createChart('myChart1', [4.3, 9.7, 8.4, 9.8, 29.7]);
...
function toggleGraph() {
  ...
  async function handlePayment(amount) {
    try {
        const response = await fetch("http://localhost:3000/create-checkout-session-nvidia", {
            method: "POST",
  ...
  document.getElementById("buyBtn").addEventListener("click", async function () {
    ...
    const response = await fetch("/create-checkout-session-nvidia", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
