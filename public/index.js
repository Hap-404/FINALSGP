...
window.onload = function () {
  ...
  revenueChart = createChart('myChart', [274, 365, 394, 383, 391]);
  profitChart = createChart('myChart1', [57, 94, 96, 99, 93]);

  document.getElementById('myChart').classList.add('active'); // Show revenue by default
};

let currentGraph = 1; // Keeps track of the current graph type
...
//   let graph = document.querySelector(".graphcont"); // Graph Container
...
// window.addEventListener("scroll", function () {
//   let buyBox = document.getElementById("buyBox"); // Buy/Sell Box
//   let graph = document.querySelector(".cont"); // Graph Container
...
//   try {
//       const response = await fetch('/stock?symbol=AAPL');
//       const data = await response.json();
