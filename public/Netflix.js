...
window.onload = function () {
  ...
  revenueChart = createChart('myChart', [20.2, 24.9, 29.7, 31.6, 39.8]);
  profitChart = createChart('myChart1', [1.9, 2.8, 5.1, 4.5, 4.4]);
...
window.addEventListener("storage", function(event) {
  ...
  async function handlePayment(amount) {
    try {
        const response = await fetch("http://localhost:3000/create-checkout-session-netflix", {
            method: "POST",
  ...
  document.getElementById("buyBtn").addEventListener("click", async function () {
    ...
    const response = await fetch("/create-checkout-session-netflix", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
